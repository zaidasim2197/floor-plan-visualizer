...
import { assertCanRelease } from "@/server/lib/bookingStateMachine";
import { audit, notify } from "@/server/lib/notify";

export const Route = createFileRoute("/api/v1/admin/bookings/$reference/release")({
  server: {
    handlers: {
      POST: async ({ request, params }: { request: Request; params: Record<string, string> }) =>
        handle(async () => {
          ...
          assertCanRelease(booking);

          booking.status = "CANCELLED";
          booking.cancelledAt = new Date();
          await booking.save();

          void audit("BOOKING_RELEASED", payload.username,
            `${booking.companyName} booking cancelled.`,
            String(booking._id), booking.reference, String(event._id));
          void notify("CUSTOMER", booking.email,
            `Reservation released — ${booking.reference}`,
            `Your reservation has been released by the organiser.`,
            String(booking._id), booking.reference, String(event._id));
