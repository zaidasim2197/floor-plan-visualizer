...
describe("Sprint Acceptance & Architectural Invariants", () => {
  ...
  before(async () => {
    ...
    testEvent = await Event.create({
      ...
      venue: { name: "Expo Center", city: "New York", address: "Main Boulevard" },
      currency: "USD",
      contact: { email: "test@vision71.test", phone: "+1 555 0000000", whatsapp: [] },
  ...
  test("Check 9: Cross-Event Isolation - Cannot book or query space from Event 1 under Event 2", async () => {
    ...
    const secondEvent = await Event.create({
      ...
      venue: { name: "Chicago Expo", city: "Chicago", address: "Downtown" },
      currency: "USD",
      contact: { email: "org@event2.test", phone: "+15550000001", whatsapp: [] },
